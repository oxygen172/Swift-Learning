...
//闭包可以捕获和存储其所在上下文中任意常量和变量的引用。 这就是所谓的闭合并包裹着这些常量和变量,俗称闭包。Swift会为你处理所有捕获动作的内存管理。
// 注意：如果您不熟悉捕获 (capturing) 这个概念也不用担心，后面Capturing Values会详细对其进行介绍。
/*
...
// 2. Closures Expressions 闭包表达式
  //嵌套函数是一种在较复杂函数中方便进行命名和定义自包含代码模块的方式。 当然,有时候撰写小巧的没有完整定义和命名的类函数结构也是很有用处的,尤其是在处理一些函数并需要将另外一些函数作为该函数的参数时。//闭包表达式是一种利用简洁语法构建内联闭包的方式。 闭包表达式提供了一些语法优化,使得撰写闭包变得简单明了。 下面闭包表达式的例子通过使用几次迭代展示了 sort 函数 定义和语法优化的方式。 每一次迭代都用更简洁的方式描述了相同的功能。



// 2.1 The Sorted Function    Sorted 函数
   //Swift 标准库提供了 sort 函数,会根据您提供的排序闭包将已知类型数组中的值进行排序。一旦排序完成,函数会返回一个与原数组大小相同的新数组,该数组中包含已经正确排序的 同类型元素。
...
// 提供排序闭包的一种方式是撰写一个符合其类型要求的普通函数，并将其作为 sorted 函数的第二个参数传入：
...
func backwards2(s1: String, s2: String) -> Bool {
    println("s1 is \(s1)") // 好奇怪为什么s1是AB
    println("s2 is \(s2)") // 好奇怪为什么s2是AA
    return s1 > s2  // B > A, 所以返回true
}

var reversed2 = sorted(names2, backwards2)


// 2.2  Closure Expression Syntax  闭包表达式语法
  // 闭包表达式语法有如下一般形式：
/*
...
//闭包表达式语法可以使用常量、变量和 inout 类型作为参数,但不提供默认值。也可以在参数列表的最后使用可变参数（就是可变形参）。元组也可以作为参数和返回值。
...
//{(s1: String, s2: String) -> Bool in return s1 > s2} 这一个就是内联闭包（inline closure）。
//需要注意的是内联闭包参数和返回值类型声明与 backwards 函数类型声明相同。在这两种方式中,都写成了 (s1: String, s2: String) -> Bool 类型。 然而在内联闭包表达式中,函数和返回值类型都写在大括号内,而不是大括号外。//闭包的函数体部分由关键字 in 引入。 该关键字表示闭包的参数和返回值类型定义已经完成,闭包函数体即将开始。
...
// 2.3 Inferring Type From Context 根据上下文推断类型
...
//实际上任何情况下,通过内联闭包表达式构造的闭包作为参数传递给函数时,都可以推断出 闭包的参数和返回值类型,这意味着您几乎不需要利用完整格式构造任何内联闭包。

//然而,你也可以使用明确的类型,如果你想它避免读者阅读可能存在的歧义,这样还是值得 鼓励的。这个排序函数例子，闭包的目的是很明确的，即排序被替换，而且对读者来说可以安全的假设闭包可能会使用字符串值,因为它正协助一个字符串数组进行排序。



// 2.4 Implicit Returns from Single-Expression Closures  单行表达式闭包可以隐藏（省略）Return
  // 单行表达式闭包可以通过隐藏 return 关键字来隐式返回单行表达式的结果，如上版本的例子可以改写为：

// var reversed3 = sorted(names, {s1, s2 in s1 > s2 }) 
// 报错！！！！！

//单行表达式闭包可以通过隐藏 return 关键字来隐式返回单行表达式的结果
//sort 函数的第二个参数函数类型明确了闭包必须返回一个 Bool 类型值。 因为闭包函数体只包含了一个单一表达式 (s1 > s2),该表达式返回 Bool 类型值,因此这里没有歧义,return 关键字可以省略。



// 2.5 Shorthand Argument Names  参数名简写
  //Swift 自动为内联函数提供了参数名称简写功能,您可以直接通过 $0,$1,$2 等名字来引用的闭包的参数的值。
//如果您在闭包表达式中使用参数名称简写,您可以在闭包参数列表中省略对其的定义,并且对应参数名称简写的类型会通过函数类型进行推断。 in 关键字也同样可以被省略
...
// 2.6 Operator Functions  运算符函数
  //实际上还有一种更简短的方式来撰写上面例子中的闭包表达式。 Swift 的 String 类型定义了关于大于号 (>) 的字符串实现,让其作为一个函数接受两个 String 类型的参数并返回 Bool 类型的值。而这正好与 sort 函数的第二个参数需要的函数类型相符合。因此,您可 以简单地传递一个大于号,Swift 可以自动推断出您想使用大于号的字符串函数实现
var reversed5 = sorted(names, >)
...
//Trailing 闭包是一个书写在函数括号()之外的闭包表达式,函数支持将其作为最后一个参数调用。

func someFunctionTakesAClosure(closure: () -> ()) {
    // fucntion body goes here 函数体部分
}

// here's how you call this function without using a trailing closure: 以下是不使用 trailing 闭包进行函数调用

someFunctionTakesAClosure( {
     // closure's body goes here 闭包主体部分
    })


// here's how you call this function with a trailing closure instead: 以下是使用 trailing 闭包进行函数调用

someFunctionTakesAClosure() {
        // trailing closure's body goes here. trailing闭包主体部分
    }

//注意:如果函数只需要闭包表达式作为唯一一个参数,当您使用 trailing 闭包时,您甚至可以把() 省略掉。
...
let numbers = [16, 58, 510,7&1] //7&1 能存入数组而成为1 应该是bug???
...
// strings is inferred to be of type [String], 常量strings被推断为字符串类型数组
...
//注意：字典 digitNames 下标后跟着一个叹号 (!)，因为字典下标返回一个可选值 (optional value)，表明即使该 key不存在也不会查找失败。 在上例中，它保证了 number % 10 可以总是作为一个 digitNames 字典的有效下标 key。 因此叹号可以用于强解析 (force-unwrap) 存储在可选下标项中的 String 类型值
...
//￼上例中 trailing 闭包语法在函数后整洁封装了具体的闭包功能,而不再需要将整个闭包包裹 在 map 函数的括号内



// 4. Capturing Values  捕获（常量/变量）数值
...
func makeIncrementor(forIncrement amount: Int) -> () -> Int {
    //makeIncrementor函数定义了一个Int形参叫amount（本地形参名），它的外部形参名叫forIncrement
    var runningTotal = 0
...
//注意:Swift 会决定捕获引用还是拷贝值。 您不需要标注 amount 或者 runningTotal 来声明 在嵌入的 incrementor 函数中的使用方式。 Swift 同时也处理 runingTotal 变量的内存 管理操作,如果不再被 incrementor 函数使用,则会被清除。


// 下面为一个使用 makeIncrementor 的例子：
let incrementByTen = makeIncrementor(forIncrement: 10)
//￼该例子定义了一个叫做 incrementByTen 的常量,该常量指向一个每次调用会加 10 的 incrementor函数。 调用这个函数多次可以得到以下结果：
...
// 5. Closures Are Reference Types  闭包是引用类型
  /*
上面的例子中,incrementBySeven 和 incrementByTen 是常量,但是这些常量指向的闭包仍然可以增加其捕获的变量值。 这是因为函数和闭包都是引用类型。 无论您将函数/闭包赋值给一个常量还是变量,您实际上都是将常量/变量的值设置为对应函数/闭包的引用。 上面的例子中,incrementByTen 指向闭包的引用是一个常量,而并非闭包内容本身。这也意味着如果您将闭包赋值给了两个不同的常量/变量,两个值都会指向同一个闭包:
*/
