...
// 1. Automatic Reference Counting 自动引用计数（ARC）
  /*
Swift 使用自动引用计数(ARC)来跟踪并管理应用使用的内存。大部分情况下,这意味着在Swift 语言中,内存管理"仍然工作",不需要自己去考虑内存管理的事情。当实例不再被使用时,ARC 会自动释放这些类的实例所占用的内存。
...
但是,如果 ARC 释放了仍在使用的实例,那么你就不能再访问这个实例的属性或者调用它的方法。如果你仍然试图访问这个实例,你的应用极有可能会崩溃。
...
class Person0 {
    ...
    init(name: String) {
        self.name = name
        println("\(name) is being inistialized")
    }
    deinit {
...
// 类Person有一个初始化函数（initializer），设置这个实例的name属性，打印一条消息来指示初始化正在进行。类Person还有一个deinitializer方法，当销毁一个类的实例时，会打印一条消息。
...
// 因为这些变量都是可选类型(Person?,而非Person),因此他们都被自动初始化为 nil,并且当前并没有引用一个 Person 的实例。

// 在我们创建一个新的 Person 实例,并且将它赋值给上述三个变量中的一个
...
//如果你通过赋值 nil 给两个变量来破坏其中的两个强引用(包括原始的引用),那就只剩下一个强引用,这个 Person 实例也不会被销毁
reference1 = nil
reference2 = nil

//直到第三个也是最后一个强引用被破坏,ARC 才会销毁 Person 的实例,这时,有一点非常明确,你无法继续使用 Person 实例:
reference3 = nil
// prints "John Appleseed is being deinitialized"  没有看到啊
...
// 4. Strong Reference Cycles Between Class Instance 类实例间得强引用环
  /*
在上面的例子中,ARC 可以追踪 Person 实例的引用数量,并且在它不再被使用时销毁这个实例。
然而,我们有可能会写出这样的代码,一个类的实例永远不会有 0 个强引用。在两个类实例彼此保持对方的强引用,使得每个实例都使对方保持有效时会发生这种情况。我们称之为强引用环。
通过用弱引用或者无主引用来取代强引用,我们可以解决强引用环问题。这个过程在Resolving Strong Reference Cycles Between Class Instances 里描述了。在开始学习如何解决这个问题之前,理解它产生的原因会很有帮助。
下面的例子展示了一个强引用环是如何在不经意之间产生的。例子定义了两个类,分别叫Person 和 Apartment,这两个类建模了一座公寓以及它的居民
...
// 糟糕的是,关联这俩实例生成了一个强引用环,Person 实例和 Apartment 实例各持有一个对方的强引用。因此,即使你破坏 john 和 number73 所持有的强引用,引用计数也不会变 为 0,因此 ARC 不会销毁这两个实例:

//john = nil
//number73 = nil

// 注意,当上面两个变量赋值为 nil 时,没有调用任何一个 deinitializer。强引用环阻止了 Person和 Apartment 实例的销毁,进一步导致内存泄漏。
...
// 5. Resolving Strong Reference Cycles Between Class Instance 解决实例间的强引用环
  // Swift 提供两种方法来解决强引用环:weak references弱引用和 unowned references无主引用。
...
// 5.1 Weak References 弱引用
  // 弱引用是一种不会被实例强引用的引用,因此不会阻止 ARC 销毁被引用的实例。这种特性使得引用不会变成强引用环。声明属性或者变量的时候,关键字 weak 表明引用为弱引用。

// 在实例的生命周期中,如果某些时候引用没有值,那么弱引用可以阻止强引用环。如果整个生命周期内引用都有值,那么相应的用无主引用,在Unowned References这一章中有详细描述。在上面的Apartment例子中,有时一个 Apartment 实例可能没有房客,因此此处应该用弱引用。

// 注意:弱引用只能声明为变量类型,因为运行时它的值可能改变。弱引用绝对不能声明为常量。

// 因为弱引用可以没有值,所以声明弱引用的时候必须是可选类型的。在 Swift 语言中,推荐 用可选类型来作为可能没有值的引用的类型。

// 如前所述,弱引用不会保持实例,因此即使实例的弱引用依然存在,ARC 也有可能会销毁实例,并将弱引用赋值为 nil。你可以想检查其他的可选值一样检查弱引用是否存在,永远 也不会碰到引用了也被销毁的实例的情况。
...
// 然后创建两个变量(john和number73)的强引用，并关联这两个实例：
var john2: Person2?
var number73_2: Apartment2?

john2 = Person2(name: "John Appleseed")
number73_2 = Apartment2(number: 73)

        
john2!.apartment = number73_2
number73_2!.tenant = john2

// 下面是引用的关系图：



//Person 的实例仍然是 Apartment 实例的强引用,但是 Apartment 实例则是 Person 实例的 弱引用。这意味着当破坏 john 变量所持有的强引用后,不再存在任何 Person 实例的强引用:
...
// number73 还持有 Apartment 实例的强引用。如果你破坏这个强引用,那么也不存在￼Apartment 实例的任何强引用
...
// 5.2 Unowned References 无主引用
  // 跟弱引用一样， 无主引用也不强持有实例。但是和弱引用不同的是,无主引用默认始终有值。因此,无主引用只能定义为非可选类型(non-optional type)。在属性、变量前添加 unowned关键字,可以声明一个无主引用。
...
Customer、CreditCard 的关系和之前弱引用例子中的 Apartment、Person 的关系截然不同。 在这个模型中,消费者不一定有信用卡,但是每张信用卡一定对应一个消费者。鉴于这种关系,Customer 类有一个可选类型属性 card,而 CreditCard 类的 customer 属性则是非可选 类型的。
...
class CreditCard {
    let number: UInt64  // 这是为了能够32位或64位系统有足够大存储16位数字
...
// 注意：CreditCard里的number属性定义为UInt64而不是Int，是为了确保它在32位和64位系统都能足够大存储16位卡片数字。

// ￼下面的代码定义了一个叫 john 的可选类型 Customer 变量,用来保存某特定消费者的引 用。因为是可变类型,该变量的初始值为 nil:
var john3: Customer?

//￼现在创建一个 Customer 实例,然后用它来初始化 CreditCard 实例,并把刚创建出来的 CreditCard 实例赋值给 Customer 的 card 属性:
john3 = Customer(name: "John Appleseed3")
...
// 此时 Customer 实例被销毁。然后,CreditCard 实例的强引用也不复存在,因此 CreditCard实例也被销毁:
    
john3 = nil
// prints "John Appleseed is being deinitialized"
// prints "Card # 1234567890123456 is being deinitialized"
        
// 上面的代码证明,john 变量赋值为 nil 后,Customer 实例和 CreditCard 实例的 deinitializer方法都打印了"deinitialized"消息。



// 5.3 Unowned References and Implicitly Unwrapped Optional Properties 无主引用以及隐式展开的可选属性
...
但是,还存在第三种场景:两个属性都必须有值,且初始化完成后不能为 nil。这种场景下,则要一个类用无主引用属性,另一个类用隐式展开的可选属性。
        
这样,在初始化完成后我们可以立即访问这两个变量(而不需要可选展开),同时又避免了引用环。本节将告诉你应该如何配置这样的关系。

*/

// 下面的例子顶一个了两个类,Country 和 City,都有一个属性用来保存另外的类的实例。在这个模型里,每个国家都有首都,每个城市都隶属于一个国家。所以,类 Country 有一个capitalCity 属性,类 City 有一个 country 属性:
...
class Country {
    ...
    init(name: String, capitalName: String){
        self.name = name
        self.capitalCity = City(name: capitalName, country: self) // 调用了City的初始化构造器
    }
}

class City {
    let name: String
    unowned let country: Country  // 无主引用属性
...
Country 的初始化函数调用了 City 的初始化函数。但是,只有 Country 的实例完全初始化完后(在 Two-Phase Initialization描述),Country 的初始化函数才能把 self 传给 City 的初始化函数。
为满足这种需求,通过在类型结尾处加感叹号(City!),我们声明 Country 的 capitalCity 属性为隐式展开的可选类型属性。就是说,capitalCity 属性的默认值是nil,不需要展开它的值 (在 Implicity Unwrapped Optionals 中描述)就可以直接访问.
...
将一个闭包赋值给类实例的某个属性,并且这个闭包使用了实例,这样也会产生强引用环。这个闭包可能访问了实例的某个属性,例如 self.someProperty,或者调用了实例的某个方法,例如 self.someMethod。这两种情况都导致了闭包使用 self,从而产生了抢引用环。
因为诸如类这样的闭包, 像类一样都是引用类型,导致了强引用环。当你把一个闭包赋值给某个属性时,你也把一个引用赋值给了这个闭包。实质上,这个之前描述的问题是一样的---两个强引用让彼此一直有效。但是,和两个类实例不同,这次一个是类实例,另一个是闭包。
Swift 提供了一种优雅的方法来解决这个问题,我们称之为闭包占用列表(closuer capture list)。同样的,在学习如何避免因闭包占用列表产生强引用环之前,先来看看这个抢引用环是如何产生的。
*/

// 下面的例子将会告诉你当一个闭包引用了 self 后是如何产生一个抢引用环的。本例顶一个 一个名为 HTMLElement 的类,来建模 HTML 中的一个单独的元素:
...
除了上面的两个属性,HTMLElement 还定义了一个 lazy 属性 asHTML。这个属性引用了一个闭包,将 name 和 text 组合成 HTML 字符串片段。该属性是() -> String 类型,就是“没有 参数,返回 String 的函数”。
默认将闭包赋值给了 asHTML 属性,这个闭包返回一个代表 HTML 标签的字符串。如果 text 值存在,该标签就包含可选值 text;或者不包含文本。对于段落,根据 text 是"some text"还 是 nil,闭包会返回"<p>some text</p>"或者"<p />"。
...
// 但是,闭包使用了 self(引用了 self.name 和 self.text),因此闭包占有了 self,这意味着闭 包又反过来持有了 HTMLElement 实例的强引用。这样就产生了强引用环。(更多闭包哪占有值的信息,请参考 Capturing Values)。
    
// 注意:虽然闭包多次使用了 self,它只占有 HTMLElement 实例的一个强引用。

// 如果设置 paragraph 为 nil,打破它持有的 HTMLElement 实例的强引用,HTMLElement 实 例和它的闭包都不会被销毁,就因为强引用环
...
// 7. Resolving Strong Reference Cycles for Closures解决闭包产生的强引用环
  /*
在定义闭包时同时定义占有列表作为闭包的一部分,可以解决闭包和类实例之间的强引用环。占有列表定义了闭包内占有一个或者多个引用类型的规则。和解决两个类实例间的强引 用环一样,声明每个占有的引用为弱引用或无主引用,而不是强引用。根据代码关系来决定使用弱引用还是无主引用。
*/
//  注意：Swift有如下约束：只要在闭包内使用self的成员，就要用self.someProperty或者self.someMethod（而非只是someProperty或someMethod）。这可以提醒你可能会不小心就占有了self。


// 7.1 Defining a Capture List 定义捕获列表
  // 捕获列表中的每个元素都是由 weak 或者 unowned 关键字和实例的引用(如 self 或someInstance)或带有某值的初始变量（例如 delegate = self.delegate）组成。每一对都在花括号中,通过逗号分开。
...
// 7.2 Weak and Unowned References 弱引用和无主引用
...
// 这一次,闭包以无主引用的形式占有 self,并不会持有 HTMLElement 实例的强引用。如果 赋值 paragraph 为 nil,HTMLElement 实例将会被销毁,并能看到它的 deinitializer 打印的 消息。
paragraph2 = nil
// prints "p is being deinitialized" // 我没有看到，奇怪
